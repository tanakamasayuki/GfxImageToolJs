...
existing configuration. A legacy `.imagesconfig` directly under the requested directory takes precedence.
...
Canonical projects write header tracking to `images/.gfx-image-tool/headers.json` and preview
tracking to `.gfx-image-tool-previews.json` in the preview output. These manifests contain only files previously
