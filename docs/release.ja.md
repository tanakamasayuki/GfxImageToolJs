...
4. Web版をChromium系とFirefoxで開き、複数画像、同名差替え、透過と背景切替（点滅を含む）、元色の
   スポイトと`source_key` export、最終結果の透過情報、変換後preview、各download、設定drop/export、
   日英切替を確認する。project ZIPをdownloadして`images/`、
   `generated/`、`previews/`の構成を確認し、新しいpageへ再dropして設定と画像が復元され、preview PNGが
   入力として登録されないことを確認する。
