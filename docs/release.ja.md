...
3. CLIで単一画像、directory bundle、透過、preview、`--check`の代表操作を確認する。
   canonical projectでは`.gfx-image-tool/`を削除しても同じheaderへ戻り、cache欠落だけなら`--check`が
   warning付きで成功することも確認する。
4. Web版をChromium系とFirefoxで開き、複数画像、同名差替え、透過と背景切替（点滅を含む）、元色の
   スポイトと`source_key` export、存在しない抜き色で透過0件へ戻ること、最終結果の透過情報、
   変換後preview、各download、設定drop/export、
   日英切替を確認する。project ZIPをdownloadしてrootの`images.h`と`images/`内の元画像、
   `.imagesconfig`、`.gitignore`を確認する。report、preview、`.gfx-image-tool/`が含まれないこと、
   新しいpageへ再dropして設定と画像が復元されることを確認する。
